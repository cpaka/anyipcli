...
Ключи также можно сохранить из панели: anyip serve → шестерёнка → Настройки.

Хранение: config.json и sessions.json — обычный JSON с правами 0600 в каталоге 0700
(только владелец), права восстанавливаются после каждой записи. Панель пишет в тот же
файл, слушает только 127.0.0.1, отклоняет запросы не по loopback-имени и никогда не
возвращает полный ключ на страницу.
...
Запустите локальный браузерный интерфейс для визуального управления:
    ...
    anyip serve               # открывает http://127.0.0.1:4747
    anyip dashboard           # та же команда — алиасы: dashboard, dash, gui
    anyip serve --port 8080   # другой порт

Скриншоты: docs/images/dashboard-proxies.png (вкладка Proxies)
           docs/images/dashboard-settings.png (окно настроек)

Нажмите Ctrl+C для остановки. Панель слушает только 127.0.0.1 и отклоняет запросы
не по loopback-имени. Интерфейс включает:
- Список аккаунтов с переключателями включения/отключения
- Форму AI-генератора
- Обзор трафика
- Просмотр сессий
- Чекбоксы для множественного выбора: копировать, экспортировать, сменить IP или удалить пакетом
- Пароли скрыты (*****) до нажатия Show passwords; копирование и экспорт берут настоящий
- Флаг страны в каждой строке, 🌐 Global если страна не задана
